@extends('dashboard.layouts.main')

@section('dashboard-content')
<div class="container mt-4">
    <div class="row mb-3">
        <div class="col-md-6">
            <h5 class="mb-4">Daftar Ketidakhadiran</h5>
        </div>
        <div class="col-md-6 d-flex justify-content-end">
            <div class="btn-wrapper">
                <button type="button" class="btn btn-primary btn-sm">
                    <i class="fa-solid fa-plus"></i> Tambah Ketidakhadiran
                </button>
            </div>
        </div>
    </div>        
    <div class="row">
        <div class="table-responsive">
            <table id="data-table" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        <th>NIP</th>
                        <th>Nama Pegawai</th>
                        <th>Tanggal Mulai</th>
                        <th>Tanggal Selesai</th>
                        <th>Tipe Ketidakhadiran</th>
                        <th>Status</th>
                        <th>Alasan</th>
                        <th>Disetujui Oleh</th>
                        <th class="aksi">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>321654987</td>
                        <td>Fani Maulida</td>
                        <td>01-10-2024</td>
                        <td>01-10-2024</td>
                        <td><span class="badge text-bg-danger">Absen</span></td>
                        <td><span class="badge text-bg-warning">Belum Disetujui</span></td>
                        <td>Sakit Flu</td>
                        <td>-</td>
                        <td class="aksi">
                            <a href="#" class="btn btn-primary btn-sm"><i class="fa-regular fa-eye"></i></a>
                            <button type="button" class="btn btn-warning btn-sm"><i class="fa-solid fa-pen-to-square"></i></button>
                            <button type="button" class="btn btn-danger btn-sm"><i class="fa-solid fa-trash"></i></button>
                        </td>
                    </tr>
                    <tr>
                        <td>123456789</td>
                        <td>Drs. Joko Susilo</td>
                        <td>02-10-2024</td>
                        <td>02-10-2024</td>
                        <td><span class="badge text-bg-warning">Izin</span></td>
                        <td><span class="badge text-bg-success">Disetujui</span></td>
                        <td>Keperluan Keluarga</td>
                        <td>Manajer Keuangan</td>
                        <td class="aksi">
                            <a href="#" class="btn btn-primary btn-sm"><i class="fa-regular fa-eye"></i></a>
                            <button type="button" class="btn btn-warning btn-sm"><i class="fa-solid fa-pen-to-square"></i></button>
                            <button type="button" class="btn btn-danger btn-sm"><i class="fa-solid fa-trash"></i></button>
                        </td>
                    </tr>
                    <tr>
                        <td>987654321</td>
                        <td>Rina Kurniawati</td>
                        <td>03-10-2024</td>
                        <td>03-10-2024</td>
                        <td><span class="badge text-bg-info">Cuti</span></td>
                        <td><span class="badge text-bg-succes">Disetujui</span></td>
                        <td>Liburan Keluarga</td>
                        <td>HRD</td>
                        <td class="aksi">
                            <a href="#" class="btn btn-primary btn-sm"><i class="fa-regular fa-eye"></i></a>
                            <button type="button" class="btn btn-warning btn-sm"><i class="fa-solid fa-pen-to-square"></i></button>
                            <button type="button" class="btn btn-danger btn-sm"><i class="fa-solid fa-trash"></i></button>
                        </td>
                    </tr>
                    <tr>
                        <td>456789123</td>
                        <td>Ahmad Setiawan</td>
                        <td>04-10-2024</td>
                        <td>04-10-2024</td>
                        <td><span class="badge text-bg-danger">Absen</span></td>
                        <td><span class="badge text-bg-warning">Belum Disetujui</span></td>
                        <td>Alasan Pribadi</td>
                        <td>-</td>
                        <td class="aksi">
                            <a href="#" class="btn btn-primary btn-sm"><i class="fa-regular fa-eye"></i></a>
                            <button type="button" class="btn btn-warning btn-sm"><i class="fa-solid fa-pen-to-square"></i></button>
                            <button type="button" class="btn btn-danger btn-sm"><i class="fa-solid fa-trash"></i></button>
                        </td>
                    </tr>
                </tbody>                                           
            </table>
        </div>
    </div>
</div>
@endsection
